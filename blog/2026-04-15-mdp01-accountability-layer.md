...
I asked whether to build the ledger as a standalone Quarkus module. My first instinct was yes — the design felt like a separate concern. Then I stopped and asked the harder question: if someone adds `quarkus-tarkus` to their application and doesn't want the ledger, do they have to carry it? Does the `WorkItemService` get `if (ledgerConfig.enabled())` guards? Does `TarkusConfig` grow a ledger section?
